...
//Kalman filterPulse(0.125, 32, 1023, 0); //based on https://github.com/bachagas/Kalman
long pulse, cm, barHeight, filteredPulse;
...
void loop() {
  ...
  // filter sensor data
  // filteredPulse = filterPulse.getFilteredValue(pulse);
  if (pulse == 0) cm = 120;
  else cm = pulse / (2*29.1);
  // cm = filteredPulse / (2*29.1);
  Serial.println(cm);
  //filteredcm = filterDistance.getFilteredValue(cm);
